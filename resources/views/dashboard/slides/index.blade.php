@extends('layouts.dashboard', ['title' => 'Liste des images', 'toolbar' => '_toolbar2', 'breadcrumb' => 'Images-Slide'])
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Liste d'images</h6>
                    <div class="dropdown morphing scale-left">
                        <a href="#" class="card-fullscreen" data-bs-toggle="tooltip" title="Card Full-Screen"><i
                                class="icon-size-fullscreen"></i></a>
                        <a href="{{ route('slides.create') }}" class="btn btn-primary d-inline">Ajouter
                            des images</a>
                    </div>
                </div>
                        ...
                        <table id="myTable" class="table myDataTable table-hover align-middle mb-0 card-table">
                            ...
                            <tbody>
                                @foreach ($slides as $index => $slide)
                                    @php
                                    $lienImage = $slide->lien_image ? asset($slide->lien_image) : asset('assets/dashboard/img/default-img.png');
                                        $statusBadge =
                                            $slide->status == 1
                                    ...
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>
                                            <img src="{{ $lienImage }}" class="avatar sm rounded me-2" alt="Image {{ $slide->categorie }}">
                                        </td>
                                        <td>
                                        ...
                                        <td>{{ $slide->categorie }}</td>
                                        <td>{!! $statusBadge !!}</td>
                                        <td>
                                            @if ($slide->status == 2)
                                                <a href="#deleteModal{{ $slide->id }}" id="RestaurerImageSlide"
                                            ...
                                            @else
                                                {{-- <a href="{{ route('slides.show', $slide->id) }}" id="ShowImageSlide"
                                                    class="btn btn-link btn-sm text-success infoIcon"
                                                    data-bs-toggle="tooltip" data-bs-toggle="modal"
                                                    data-bs-target="#info_imageSlide" data-bs-placement="top" title="Infos"><i
                                                        class="fa fa-eye"></i></a> --}}
                                                <a href="{{ route('slides.edit', $slide->id) }}" id="EditImageSlide"
                                                    class="btn btn-link btn-sm text-primary editIcon" data-bs-toggle="tooltip"
                                                    data-bs-target="#edit_imageSlide" title="Modifier"><i
                                                        class="fa fa-pencil"></i></a>
                                                <a href="#deleteModal{{ $slide->id }}" id="DeleteImageSlide"
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-body p-5 text-center">
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        @if ($slide->status == 2)
                                                            <h4>Vous êtes sur le point de restaurer <br>cette image ?</h4>
                                                            {{-- <p class="text-muted fs-15 mb-4">En restaurant cette image, vous
                                                                ramener
                                                                <br> toutes les informations la concernant de notre base de
                                                                données.
                                                            </p> --}}
                                                        @else
                                                            <h4>Vous êtes sur le point de supprimer <br>cette image ?</h4>
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            ...
                                                            @if ($slide->status == 2)
                                                                <form method="POST"
                                                                    action="{{ route('slide.restaure', $slide->id) }}">
                                                                    @method('PUT')
                                                                    @csrf
