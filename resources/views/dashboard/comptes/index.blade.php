@extends('layouts.dashboard', ['title' => 'Liste des comptes', 'toolbar' => '_toolbar2', 'breadcrumb' => 'Compte'])
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Liste des comptes</h6>
                            ...
                            <tbody>
                                @foreach ($comptes as $index => $compte)
                                    ...
                                    <tr>
                                        ...
                                        <td style="white-space: pre-line;">{{ formatGender($compte->mutualiste->genre) }} {{ $compte->mutualiste->nom }} {{ $compte->mutualiste->prenom }}</td>
                                        <td>{{ $compte->solde }}</td>
                                        <td>{{ $compte->quota }}</td>
                                        <td>{!! $statusBadge !!}</td>
                                        <td>
                                            ...
                                            @if ($compte->status == 2)
                                                {{-- <a href="#deleteModal{{ $compte->id }}" id="RestaurerCompte"
                                            ...
                                            @else
                                                <a href="{{ route('comptes.show', $compte->id) }}" id="ShowGrade"
                                                    class="btn btn-link btn-sm text-success infoIcon"
                                                    data-bs-toggle="tooltip" data-bs-toggle="modal"
                                                    data-bs-target="#info_type_piece" data-bs-placement="top" title="Infos"><i
                                                        class="fa fa-eye"></i></a>
                                                {{-- <a href="{{ route('comptes.edit', $compte->id) }}" id="EditCompte"
                                                    class="btn btn-link btn-sm text-primary editIcon"
                                                    data-bs-target="#edit_type_piece" title="Modifier"><i
                                                        class="fa fa-pencil"></i></a> --}}
                                                {{-- <a href="#deleteModal{{ $compte->id }}" id="DeleteCompte"
                                    ...
                                    <!-- Modal delete-->
                                    <div class="modal fade flip" id="deleteModal{{ $compte->id }}" tabindex="-1"
                                        aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-body p-5 text-center">
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        @if ($compte->status == 2)
                                                            <h4>Vous êtes sur le point de restaurer <br>un compte ?</h4>
                                                            <p class="text-muted fs-15 mb-4">En restaurant ce compte, vous
                                                                ramener
                                                                <br> toutes les informations le concernant de notre base de
                                                                données.
                                                            </p>
                                                        @else
                                                            <h4>Vous êtes sur le point de supprimer <br>un compte ?</h4>
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            ...
                                                            @if ($compte->status == 2)
                                                                <form method="POST"
                                                                    action="{{ route('compte.restaure', $compte->id) }}">
                                                                    @method('PUT')
                                                                    @csrf
                                                            ...
                                                            @else
                                                                <form method="POST"
                                                                    action="{{ route('comptes.destroy', $compte->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
