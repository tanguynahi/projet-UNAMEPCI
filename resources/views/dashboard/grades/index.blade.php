@extends('layouts.dashboard', ['title' => 'Liste des grades', 'toolbar' => '_toolbar2', 'breadcrumb' => 'Grades'])
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Liste des grades</h6>
                    <div class="dropdown morphing scale-left">
                        <a href="#" class="card-fullscreen" data-bs-toggle="tooltip" title="Card Full-Screen"><i
                                class="icon-size-fullscreen"></i></a>
                        <a href="{{ route('grades.create') }}" class="btn btn-primary d-inline">Ajouter un grade</a>
                    </div>
                </div>
                                ...
                                @foreach ($grades as $index => $grade)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span>{{ $grade->libelle }}</span>
                                        </td>
                                        <td style="white-space: pre-line;">{{ $grade->description }}</td>

                                        <td>{!! $statusBadge !!}</td>
                                        <td>

                                            @if ($grade->status == 2)
                                                <a href="#deleteModal{{ $grade->id }}" id="RestaurerGrade"
                                            ...
                                            @else
                                                {{-- <a href="{{ route('grades.show', $grade->id) }}" id="ShowGrade"
                                                    class="btn btn-link btn-sm text-success infoIcon"
                                                    data-bs-toggle="tooltip" data-bs-toggle="modal"
                                                    data-bs-target="#info_type_piece" data-bs-placement="top"
                                                    title="Infos"><i class="fa fa-eye"></i></a> --}}
                                                <a href="{{ route('grades.edit', $grade->id) }}" id="EditGrade"
                                                ...
                                                @if (Auth::user()->hasRole('super-administrateur'))
                                                    <a href="#deleteModal{{ $grade->id }}" id="DeleteGrade"
                                    ...
                                    <!-- Modal delete-->
                                    <div class="modal fade flip" id="deleteModal{{ $grade->id }}" tabindex="-1"
                                        aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-body p-5 text-center">
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        @if ($grade->status == 2)
                                                            <h4>Vous êtes sur le point de restaurer <br>un grade ?</h4>
                                                            <p class="text-muted fs-15 mb-4">En restaurant ce grade, vous
                                                                ramener
                                                                <br> toutes les informations le concernant de notre base de
                                                                données.
                                                            </p>
                                                        @else
                                                            <h4>Vous êtes sur le point de supprimer <br>un grade ?</h4>
                                                            <p class="text-muted fs-15 mb-4">En supprimant ce grade, vous
                                                                supprimez
                                                                <br> toutes les informations le concernant de notre base de
                                                                données.
                                                            </p>
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            ...
                                                            @if ($grade->status == 2)
                                                                <form method="POST"
                                                                    action="{{ route('grade.restaure', $grade->id) }}">
                                                                    @method('PUT')
                                                                    @csrf
