        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Informations sur la demande : {{ $demandeAccompagnement->service->libelle }}
                    </h6>
                    ...
                    <div class="row mb-5">
                        <div class="col-lg-6 col-md-6 col-sm-6">
                            <div class="row mb-3">
                                ...
                                {{-- <img src="{{asset( $demandeAccompagnement->mutualiste->lien_photo) }}" class="img-thumbnail"
                                    style="height: 80px; width: 100px;" alt="Image Artisan">
                                    {{ $demandeAccompagnement->mutualiste->nom }} --}}
                                <div class="col-lg-6 col-md-6 col-sm-6">
                                    {{-- <h4 class="mt-4 mt-lg-0">Numero : <strong>{{ $demandeAccompagnement->contact_tresormoney }}</strong>
                                    </h4> --}}
                                    <p class="mb-0 text-primary fs-6 fw-bold">
                                        <span class="fs-6 text-muted fw-light  me-2">Montant voulue :
                                        </span>{{ formatMontant($demandeAccompagnement->montant_voulue) }}
                                    </p>

                                    <p class="mb-0">
                                        Contact : <a href="tel:{{ $demandeAccompagnement->contact_tresormoney }}"
                                            class="fw-bold text-primary fs-6">
                                            {{ $demandeAccompagnement->contact_tresormoney }}</a>
                                    </p>
                                    <p class="mb-0 text-primary fs-6 fw-bold">
                                        <span class="fs-6 text-muted fw-light  me-2">A rembourser :</span> <span
                                            class="fs-6 fw-normal">{{ formatMontant($demandeAccompagnement->montant_apayer) }}
                                        </span>
                                    </p>
                                    <p>
                                    <h6><u>Commentaire du mutualiste:</u></h6>
                                    ...
                                    <div class="d-flex">
                                        @if ($demandeAccompagnement->status == 2)
                                            <a id="btn_rejeter" class="btn btn-outline-danger mx-2 fw-bold"><i
                                                ...
                                                Rejetée</a>
                                            <a id="btn_valider_approbation" class="btn btn-outline-success mx-2 fw-bold"><i
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            <button
                                                                ...
                                                                Annuler</button>
                                                            <form
                                                                action="{{ route('accepter.demandePret', $demandeAccompagnement->id) }}"
                                                                method="post">
                                                                @csrf
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            ...
                                                            <button type="submit" class="btn btn-primary w-25 mx-2"
                                                                id="validerButton">Valider</button>
                            ...
                            <div class="row align-items-center">
                                <div class="col-6">
                                    ...
                                    <p><i class="fa fa-calendar me-1"></i> Soumit le : <span
                                            class="fw-bold">{{ formatDateTime($demandeAccompagnement->created_at) }}</span>
                                    </p>
                                </div>
                                <div class="col-6">
                            ...
                            @if ($demandeAccompagnement->status == 3)
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        <p class="fs-6">{{ $demandeAccompagnement->rejet ?? '' }}</p>
                                    </div>
                                </div>
                            @endif
                            @if ($demandeAccompagnement->status == 2)
                                <form action="{{ route('refus.pretDemander', $demandeAccompagnement->id) }}" method="POST"
                                    id="rejeter_demande_produit" style="display: none">
                                    @csrf
                                    @method('POST')
                                    <div class="row align-items-center">
                                        <div class="col-10">
                                            ...
                                            <textarea name="rejet" id="commentaire" class="form-control no-resize @error('commentaire') is-invalid @enderror"
                                                rows="4" placeholder="Entrer le motif" autocomplete="commentaire" autofocus required>{{ old('rejet') }}</textarea>
                                            @error('commentaire')
                                    ...
                                    <div class="row g-3 ">
                                        <div class="col-10">
                                            <div class="mx-auto d-flex justify-content-center">
                                                ...
                                                {{-- <button type="submit" class="btn btn-primary w-25 mx-2" >Valider</button> --}}
                                            </div>
                                        </div>
                                    </div>
                                </form>
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6">
                            <div class="row mb-5">
                                ...
                                <img src="/{{ $demandeAccompagnement->mutualiste->lien_photo }}" class="img-thumbnail"
                                    style="height: 80px; width: 100px;" alt="Image du mutualiste">

                                <div class="col-lg-6 col-md-6">

                                    <h5 class="mt-4 mt-lg-0">{{ $demandeAccompagnement->mutualiste->nom }}
                                        {{ $demandeAccompagnement->mutualiste->prenom }}
                                    </h5>
                                    <p class="mb-0 text-primary fs-6 fw-bold">
                                        <i class="fa fa-envelope text-muted"></i> <a class=""
                                            href="mailto:{{ $demandeAccompagnement->mutualiste->user->email }}">
                                            {{ $demandeAccompagnement->mutualiste->user->email }}</a>
                                    </p>
                                    <p class="mb-0 text-primary fs-6 fw-bold">
                                        <i class="fa fa-phone text-muted"></i> <a
                                            href="tel:{{ $demandeAccompagnement->mutualiste->contact }}">
                                            {{ $demandeAccompagnement->mutualiste->contact }}</a>
                                    </p>
                                </div>
                            </div>
                        </div>
...
@push('js')
    ...
    <script>
        document.getElementById('validerButton').addEventListener('click', function() {
            document.getElementById('rejeter_demande_produit').submit();
        });
    </script>
@endpush
