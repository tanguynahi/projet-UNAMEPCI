@extends('layouts.dashboard', ['title' => 'Liste des villes', 'toolbar' => '_toolbar2', 'breadcrumb' => 'Villes'])
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Liste des villes</h6>
                    <div class="dropdown morphing scale-left">
                        <a href="#" class="card-fullscreen" data-bs-toggle="tooltip" title="Card Full-Screen"><i
                                class="icon-size-fullscreen"></i></a>
                        <a href="{{ route('villes.create') }}" class="btn btn-primary d-inline">Ajouter une
                            ville</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($villes as $index => $ville)
                                    ...
                                    <tr>
                                        ...
                                        <td style="white-space: pre-line;">{{ $ville->description }}</td>

                                        <td>{!! $statusBadge !!}</td>
                                        <td>

                                            @if ($ville->status == 2)
                                                <a href="#deleteModal{{ $ville->id }}" id="RestaurerVille"
                                            ...
                                            @else
                                                {{-- <a href="{{ route('villes.show', $ville->id) }}" id="ShowVille"
                                                    ...
                                                    data-bs-target="#info_admin" data-bs-placement="top" title="Infos"><i
                                                        class="fa fa-eye"></i></a> --}}
                                                <a href="{{ route('villes.edit', $ville->id) }}" id="EditVille"
                                                    class="btn btn-link btn-sm text-primary editIcon"
                                                    data-bs-toggle="tooltip" data-bs-target="#edit_admin"
                                                    title="Modifier"><i class="fa fa-pencil"></i></a>
                                                @if (Auth::user()->hasRole('super-administrateur'))
                                                    <a href="#deleteModal{{ $ville->id }}" id="DeleteVille"
                                    ...
                                    <!-- Modal delete-->
                                    <div class="modal fade flip" id="deleteModal{{ $ville->id }}" tabindex="-1"
                                        aria-hidden="true">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-body p-5 text-center">
                                                    ...
                                                    <div class="mt-4 text-center">
                                                        @if ($ville->status == 2)
                                                            <h4>Vous êtes sur le point de restaurer <br>une ville ?</h4>
                                                            <p class="text-muted fs-15 mb-4">En restaurant cette ville, vous
                                                                ramener
                                                                <br> toutes les informations la concernant de notre base de
                                                                données.
                                                            </p>
                                                        @else
                                                            <h4>Vous êtes sur le point de supprimer <br>une ville ?</h4>
                                                            <p class="text-muted fs-15 mb-4">En supprimant cette ville, vous
                                                                supprimez
                                                        ...
                                                        <div class="hstack gap-2 justify-content-center remove">
                                                            ...
                                                            @if ($ville->status == 2)
                                                                <form method="POST"
                                                                    action="{{ route('ville.restaure', $ville->id) }}">
                                                                    @method('PUT')
                                                                    @csrf
                                                            ...
                                                            @else
                                                                <form method="POST"
                                                                    action="{{ route('villes.destroy', $ville->id) }}">
                                                                    @csrf
                                                                    @method('DELETE')
