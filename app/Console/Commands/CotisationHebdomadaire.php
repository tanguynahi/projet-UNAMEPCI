...
class CotisationHebdomadaire extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cotisation:hebdomadaire';
     ...
     * @var string
     */
    protected $description = 'Generer automatiquement une cotisation hebdomadaires pour les mutualistes';
    ...
    public function handle()
    {
        ...
        $mutualistes = CotisationMutualiste::where('frequence_paiement', 'Hebdomadaire')
            ->whereIn('status', [1, 2])
        ...
        $facturations = Facturation::where('periode_id', 3)
            ->whereIn('status', [1, 3])
            ->where('date_fin', '>=', Carbon::today())
        ...
        $this->info('Cotisations hebdomadaire générées avec succès.');
    }
}
